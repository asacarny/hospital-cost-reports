...
* Once you know the worksheet, row, and column of the variable, open up `misc/lookup.xlsx` and add the info as a new row to the worksheet `Lookup Table`. Make sure that `clmn_num` and `line_num` are both stored as text format (i.e. the leading zeroes should appear). Set `fmt` to `10` the variable is from 2010 format reports and `96` if it's from 1996 format reports. Make sure to set `enabled` to 1.
* Finally, add a new row to the worksheet `Type and Label`. There are three options for `type`.
